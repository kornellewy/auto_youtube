...
def apply_transitions(image_clip, anim_type: str):
    """
    Unified transition helper.
    Slides are initialised once per direction, then re-used.
    """
    # ---------- singleton instances ----------
    fade_in = FadeIn(duration=1)
    fade_out = FadeOut(duration=1)
    cross_fade_in = CrossFadeIn(duration=1)
    cross_fade_out = CrossFadeOut(duration=1)

    # pre-build slide objects for all 4 directions
    slide_in = {
        d: SlideIn(duration=1, side=d) for d in ("left", "right", "top", "bottom")
    }
    slide_out = {
        d: SlideOut(duration=1, side=d) for d in ("left", "right", "top", "bottom")
    }

    # ---------- fades ----------
    if anim_type == "fadein":
        return fade_in.apply(image_clip)
    if anim_type == "fadeout":
        return fade_out.apply(image_clip)
    if anim_type == "fadein+fadeout":
        return fade_out.apply(fade_in.apply(image_clip))

    # ---------- cross-fades ----------
    if anim_type == "crossfadein":
        return cross_fade_in.apply(image_clip)
    if anim_type == "crossfadeout":
        return cross_fade_out.apply(image_clip)

    # ---------- slides ----------
    if anim_type.startswith("slidein_"):
        side = anim_type.split("_", 1)[1]
        return slide_in[side].apply(image_clip)

    if anim_type.startswith("slideout_"):
        side = anim_type.split("_", 1)[1]
        return slide_out[side].apply(image_clip)

    # no animation
    return image_clip


# def add_rotating_transitions(video_clips, duration=1):
#     new_video_clips = []
#     anim_types_in = [
#         "fadein",
#         "crossfadein",
...
#     #     # only transition out
#     #     video_clip = video_clip.subclip(0, video_clip.duration - duration) if video_clip.duration > duration else video_clip
#     #     if idx == 0:
#     #         video_clip = apply_transitions(
#     #             video_clip, anim_types_in[idx % len(anim_types_in)]
#     #         )
#     #         new_video_clips.append(video_clip)
#     #     else:
#     #         video_clip = apply_transitions(
#     #             video_clip, anim_types_in[idx % len(anim_types_in)]
#     #         )
#     #         video_clip = apply_transitions(
#     #             video_clip, anim_types_out[idx % len(anim_types_out)]
#     #         )
#     #         new_video_clips.append(video_clip)
#     # return new_video_clips
#     out = []
#     for idx, clip in enumerate(video_clips):
#         # 1. shorten clip by transition time
#         clip = (
#             clip.subclipped(0, clip.duration - duration)
#             if clip.duration > duration
#             else clip
#         )

#         # 2. add IN transition to every clip
#         clip = apply_transitions(clip, anim_types_in[idx % len(anim_types_in)])
#         out.append(clip)

#     # 3. add OUT transition to **last** clip only
#     out[-1] = apply_transitions(out[-1], anim_types_out[-1 % len(anim_types_out)])
#     return out


# def add_rotating_transitions(video_clips):
#     if not video_clips:
#         return video_clips

#     # pre-build 1-second transition clips (black → transparent) for each effect
#     anim_in = [
#         "fadein",
#         "crossfadein",
#         "slidein_left",
...
#     trans_clips = {}
#     for fx in anim_in + anim_out:
#         trans = apply_transitions(
#             ColorClip(size=video_clips[0].size, color=(0, 0, 0), duration=1), fx
#         )
#         trans_clips[fx] = trans.with_duration(1)

#     final_clips = []
#     for idx, clip in enumerate(video_clips):
#         # first clip gets only IN transition
#         if idx == 0:
#             final_clips.append(
#                 concatenate_videoclips(
#                     [trans_clips[anim_in[0]], clip],
#                     method="chain",
#                     transition=trans_clips[anim_in[0]],
#                 )
#             )
#             continue

#         # all others: OUT (previous) + IN (current) via overlap
#         out_fx = anim_out[(idx - 1) % len(anim_out)]
#         in_fx = anim_in[idx % len(anim_in)]
#         pair = concatenate_videoclips(
#             [final_clips[-1], clip],
#             method="chain",
#             transition=trans_clips[out_fx],  # overlap uses previous OUT
#         )
#         final_clips[-1] = pair  # replace tail with glued segment

#     return final_clips


def apply_animation(image_clip, anim_type: str):
    if anim_type == "zoomin":
        return Zoom(image_clip, mode="in", position="center", speed=1.1)
...
def build_meme_clip(
    ...
    # TODO: memes should be like change every 3 seconds so we need to calculate how many memes we need and then do it in loop
    MEME_CHANGE_EVERY = 3.0  # seconds
    num_memes = max(1, int(final_duration // MEME_CHANGE_EVERY))
    single_meme_time = final_duration / num_memes
    ...
    for meme_idx in range(num_memes):
        ...
        if ext in [".gif", ".mp4", ".mov"]:  # support GIF/video memes
            ...
            meme_clip=meme_clip.with_effects(
                [vfx.Loop(duration=single_meme_time)]
            )
        else:
            ...
            meme_clip = ImageClip(meme).with_duration(single_meme_time)
        meme_clips.append(meme_clip)

    meme_clip = concatenate_videoclips(meme_clips)

    # --- 6. Adjust duration ---
    final_duration = audio_clip.duration
    meme_clip = meme_clip.with_duration(final_duration)
    background_copy = background.copy()
    background_copy = background_copy.with_effects([vfx.Loop(duration=final_duration)])
...
def build_video_blocks_from_article(blocks, config, script_name):
    ...
    for block_idx, block in enumerate(blocks):
        ...
        # add meme or filler
            ...
            # 1/2 mem to filler 
            if bool(biased_coin(block_idx, 0, len(blocks))): 
                ...
                stock_clip = VideoFileClip(random.choice(STROCK_MOVIES_PATHS))
                stock_clip=stock_clip.with_effects(
                    [vfx.Loop(duration=audio_clip.duration)]
                )
                stock_clip = stock_clip.with_duration(audio_clip.duration)
                background_copy = background.copy()
                background_copy = background_copy.with_effects([vfx.Loop(duration=audio_clip.duration)])
                background_copy.layer = 0
                stock_clip.layer = 1
                stock_clip = stock_clip.with_position(("center", "center"), relative=True)
                final_clip = CompositeVideoClip([background_copy, stock_clip])
                final_clip = final_clip.with_audio(audio_clip.with_duration(audio_clip.duration))
